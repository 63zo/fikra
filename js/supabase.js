...
const SupabaseService = {
  ...
  init() {
    ...
    try {
      ...
      // 3. Robust Multi-Device Polling Fallback (sync every 4 seconds)
      this.startPolling();
    } catch (e) {
  ...
  updateStatus(status, message) {
    ...
    if (badge) {
      if (status === 'connected') {
        ...
        badge.innerHTML = `<span class="pulse-dot"></span><span>${I18N.currentLang === 'ar' ? 'سحابي متصل' : 'Cloud Synced'}</span><i class="mdi mdi-refresh text-xs"></i>`;
  ...
  async syncAllFromRemote(silent = false) {
    ...
    try {
      ...
      // 5. Sync Ideas
      const { data: remoteDbIdeas, error: ideasErr } = await this.client.from('ideas').select('*').order('created_at', { ascending: false });
      if (!ideasErr && remoteDbIdeas && remoteDbIdeas.length > 0) {
        const mappedRemote = remoteDbIdeas.map(i => this.mapDbToIdea(i));
        
        // Detect if new items or modifications arrived
        const currentIdeasMap = new Map(State.ideas.map(i => [i.id, i]));
        let hasNewIdeas = false;
        let hasUpdates = false;

        for (const rIdea of mappedRemote) {
          const existing = currentIdeasMap.get(rIdea.id);
          if (!existing) {
            hasNewIdeas = true;
          } else {
            // Check if votes or comments changed
            const rVotes = rIdea.votes ? rIdea.votes.length : 0;
            const exVotes = existing.votes ? existing.votes.length : 0;
            const rComments = rIdea.comments ? rIdea.comments.length : 0;
            const exComments = existing.comments ? existing.comments.length : 0;
            if (rVotes !== exVotes || rComments !== exComments || rIdea.status !== existing.status) {
              hasUpdates = true;
            }
          }
        }

        if (hasNewIdeas || hasUpdates || State.ideas.length !== mappedRemote.length) {
          State.ideas = mappedRemote;
          State.saveToStorage();

          if (hasNewIdeas && !silent && window.App) {
            window.App.showToast(I18N.currentLang === 'ar' ? '💡 تم تحديث قائمة الأفكار ومزامنتها سحابياً' : '💡 Ideas synced with cloud database', 'info');
          }

          // Live update active views
          if (window.App) {
            ...
            } else if (window.App.currentView === 'profile') {
              window.App.render();
            }
          }
        }
  ...
  async seedRemoteDatabase() {
    ...
    try {
      ...
      if (State.departments.length > 0) {
        const deptsDb = State.departments.map(d => ({ id: String(d.id), name_ar: d.nameAr, name_en: d.nameEn, code: d.code || '' }));
        await this.client.from('departments').upsert(deptsDb);
      }

      // Seed job titles
      if (State.jobTitles.length > 0) {
        const jobsDb = State.jobTitles.map(j => ({ id: String(j.id), title_ar: j.titleAr, title_en: j.titleEn }));
        await this.client.from('job_titles').upsert(jobsDb);
      }

      // Seed categories
      if (State.categories.length > 0) {
        const catsDb = State.categories.map(c => ({ id: String(c.id), name_ar: c.nameAr, name_en: c.nameEn, icon: c.icon || '💡', color: c.color || '#14573A' }));
  ...
  async upsertDepartment(dept) {
    ...
    try {
      await this.client.from('departments').upsert({ id: String(dept.id), name_ar: dept.nameAr, name_en: dept.nameEn, code: dept.code || '' });
    } catch (e) {
      console.warn(e);
    }
  },
  ...
  async deleteDepartment(deptId) {
    if (!this.client) return;
    try {
      await this.client.from('departments').delete().eq('id', String(deptId));
    } catch (e) {
      console.warn(e);
    }
  },
  ...
  async upsertJobTitle(job) {
    if (!this.client) return;
    try {
      await this.client.from('job_titles').upsert({ id: String(job.id), title_ar: job.titleAr, title_en: job.titleEn });
    } catch (e) {
      console.warn(e);
    }
  },
  ...
  async deleteJobTitle(jobId) {
    if (!this.client) return;
    try {
      await this.client.from('job_titles').delete().eq('id', String(jobId));
    } catch (e) {
      console.warn(e);
    }
  },
  ...
  async upsertCategory(cat) {
    if (!this.client) return;
    try {
      await this.client.from('categories').upsert({ id: String(cat.id), name_ar: cat.nameAr, name_en: cat.nameEn, icon: cat.icon, color: cat.color });
    } catch (e) {
      console.warn(e);
    }
  },
  ...
  async deleteCategory(catId) {
    if (!this.client) return;
    try {
      await this.client.from('categories').delete().eq('id', String(catId));
    } catch (e) {
      console.warn(e);
    }
  },
      ...
      this.realtimeChannel = this.client
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'ideas' }, payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            State.ideas = State.ideas.filter(i => i.id !== String(payload.old.id));
            State.saveToStorage();
            if (window.App && window.App.currentView === 'home' && window.IdeasManager) {
              window.IdeasManager.refreshFeed();
            }
